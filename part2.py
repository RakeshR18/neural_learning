...
class LinearModel:
    def __init__(self, num_inputs, learning_rate):
        """
        Model is very similar to the Perceptron, except that:
    ...
    def activation(self, x):
        ...
        """
        try :
            if (type(x) is bool or type(x) is np.ndarray or type(x) is list):
                raise ValueError
            y = 1/(1+np.exp(-x))
        except ValueError:
    ...
    def forward(self, inputs):
        ...
        """
        forward_weights = np.dot(inputs,self.weights[0:2]) + self.weights[2]
        
        return self.activation(forward_weights)
    ...
    def loss(prediction, label):
        ...
        """
        loss_value = -label * np.log(prediction) - (1 - label) * np.log(1 - prediction)
        return loss_value
    ...
    def backward(self, inputs, diff):
        ...
        updated_weight = inputs * diff * self.lr
        self.weights = np.add(self.weights, updated_weight)
